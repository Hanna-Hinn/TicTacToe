...
public class Board {
    ...
    private void initialiseBoard() {
        for (int row = 0; row < BOARD_WIDTH; row++) {
            for (int col = 0; col < BOARD_WIDTH; col++) {
                board[row][col] = winningMark.BLANK;
            }
        }
    }
    ...
    public boolean placeMark(int row, int col) {
        ...
        availableMoves--;
        board[row][col] = crossTurn ? winningMark.X : winningMark.O;
        togglePlayer();
        checkWin(row, col);
        return true;
    }
    ...
    private void checkWin(int row, int col) {
        ...
        if (calcWinner(rowSum) != winningMark.BLANK) {
    ...
    private Mark calcWinner(int rowSum) {
        int Xwin = winningMark.X.getMark() * BOARD_WIDTH;
        int Owin = winningMark.O.getMark() * BOARD_WIDTH;
        if (rowSum == Xwin) {
            gameOver = true;
            winningMark = winningMark.X;
            return winningMark.X;
        } else if (rowSum == Owin) {
            gameOver = true;
            winningMark = winningMark.O;
            return winningMark.O;
        }
        return winningMark.BLANK;
    }
